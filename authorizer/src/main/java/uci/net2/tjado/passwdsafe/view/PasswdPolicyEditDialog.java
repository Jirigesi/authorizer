 ...
 */
package uci.net2.tjado.passwdsafe.view;
...
import uci.net2.tjado.passwdsafe.file.PasswdPolicy;
import uci.net2.tjado.passwdsafe.lib.PasswdSafeUtil;
import uci.net2.tjado.passwdsafe.lib.view.AbstractDialogClickListener;
import uci.net2.tjado.passwdsafe.lib.view.TypefaceUtils;
...
public class PasswdPolicyEditDialog extends AppCompatDialogFragment
{
    ...
    Dialog onCreateDialog(Bundle savedInstanceState)
    {
        ...
        itsView = factory.inflate(uci.net2.tjado.passwdsafe.R.layout.passwd_policy_edit, null);

        SharedPreferences prefs = Preferences.getSharedPrefs(ctx);

        itsIsNameEditable = true;
        itsNameEdit = (TextView)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.name);
        itsLengthEdit = (TextView)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.length);
        itsOptions[0] = (CheckBox)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.lowercase);
        itsOptions[1] = (CheckBox)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.uppercase);
        itsOptions[2] = (CheckBox)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.digits);
        itsOptions[3] = (CheckBox)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.symbols);
        itsOptionLens[0] = (TextView)itsView.findViewById(
                uci.net2.tjado.passwdsafe.R.id.lowercase_len);
        itsOptionLens[1] = (TextView)itsView.findViewById(
                uci.net2.tjado.passwdsafe.R.id.uppercase_len);
        itsOptionLens[2] = (TextView)itsView.findViewById(
                uci.net2.tjado.passwdsafe.R.id.digits_len);
        itsOptionLens[3] = (TextView)itsView.findViewById(
                uci.net2.tjado.passwdsafe.R.id.symbols_len);
        itsUseCustomSymbols =
            (CheckBox)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.use_custom_symbols);
        itsCustomSymbolsEdit =
            (TextView)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.symbols_custom);
        TypefaceUtils.setMonospace(itsCustomSymbolsEdit, ctx);
        itsGeneratedPasswd =
                (TextView)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.generated_passwd);
        ...
        if (itsPolicy != null) {
            ...
            titleId = uci.net2.tjado.passwdsafe.R.string.edit_policy;
            ...
            titleId = uci.net2.tjado.passwdsafe.R.string.new_policy;
            name = "";
            len = 12;
        ...
        AlertDialog.Builder alert = new AlertDialog.Builder(ctx)
            .setTitle(titleId)
            .setView(itsView)
            .setPositiveButton(uci.net2.tjado.passwdsafe.R.string.ok, dlgClick)
            .setNegativeButton(uci.net2.tjado.passwdsafe.R.string.cancel, dlgClick)
        ...
        View v = itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.name_row);
        ...
        Button btn = (Button)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.generate);
    ...
    private void setType(PasswdPolicy.Type type, boolean init)
    {
        ...
        if (init) {
            Spinner typeSpin = (Spinner)itsView.findViewById(
                    uci.net2.tjado.passwdsafe.R.id.type);
        ...
        setVisible(uci.net2.tjado.passwdsafe.R.id.lowercase_row, optionsVisible);
        setVisible(uci.net2.tjado.passwdsafe.R.id.uppercase_row, optionsVisible);
        setVisible(uci.net2.tjado.passwdsafe.R.id.digits_row, optionsVisible);
        setVisible(uci.net2.tjado.passwdsafe.R.id.symbols_row, optionsVisible);
        for (CheckBox option: itsOptions) {
            setOptionLenVisible(option);
        }
        setCustomSymbolsVisible();
        setTextView(uci.net2.tjado.passwdsafe.R.id.symbols_default, defaultSymbols);
    ...
    private void setOption(CheckBox option, boolean use, boolean init)
    {
        ...
        setOptionLenVisible(option);
        if (option.getId() == uci.net2.tjado.passwdsafe.R.id.symbols) {
            setCustomSymbolsVisible();
        }
    ...
    private void setCustomSymbolsOption(boolean useCustom, boolean init)
    {
        ...
        View defView = itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.symbols_default);
        if (useCustom) {
    ...
    private void setCustomSymbolsVisible()
    {
        ...
        switch (itsType) {
        case NORMAL: {
            CheckBox cb = (CheckBox)itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.symbols);
        ...
        setVisible(uci.net2.tjado.passwdsafe.R.id.custom_symbols_set, visible);
    }
    ...
    private void setOptionLenVisible(CheckBox option)
    {
        ...
        int lengthId = 0;
        switch (option.getId()) {
        case uci.net2.tjado.passwdsafe.R.id.lowercase: {
            labelId = uci.net2.tjado.passwdsafe.R.id.lowercase_label;
            lengthId = uci.net2.tjado.passwdsafe.R.id.lowercase_len;
            break;
        }
        case uci.net2.tjado.passwdsafe.R.id.uppercase: {
            labelId = uci.net2.tjado.passwdsafe.R.id.uppercase_label;
            lengthId = uci.net2.tjado.passwdsafe.R.id.uppercase_len;
            break;
        }
        case uci.net2.tjado.passwdsafe.R.id.digits: {
            labelId = uci.net2.tjado.passwdsafe.R.id.digits_label;
            lengthId = uci.net2.tjado.passwdsafe.R.id.digits_len;
            break;
        }
        case uci.net2.tjado.passwdsafe.R.id.symbols: {
            labelId = uci.net2.tjado.passwdsafe.R.id.symbols_label;
            lengthId = uci.net2.tjado.passwdsafe.R.id.symbols_len;
            break;
        }
        }
    ...
    private final class Validator extends DialogValidator.AlertCompatValidator
    {
        ...
        {
            View generateRow = itsView.findViewById(uci.net2.tjado.passwdsafe.R.id.generate_row);
            generateRow.setVisibility(View.GONE);

            if (itsIsNameEditable) {
                String name = itsNameEdit.getText().toString();
                if (TextUtils.isEmpty(name)) {
                    return getString(uci.net2.tjado.passwdsafe.R.string.empty_name);
                }
                ...
                if ((itsPolicy == null) ||
                    ...
                    if (listener.isDuplicatePolicy(name)) {
                        return getString(uci.net2.tjado.passwdsafe.R.string.duplicate_name);
                    }
                }
            }
            ...
            try {
                ...
                if (length < 4) {
                    return getString(uci.net2.tjado.passwdsafe.R.string.length_min_val, 4);
                } else if (length > 1024) {
                    return getString(uci.net2.tjado.passwdsafe.R.string.length_max_val, 1024);
                } else if ((itsType == PasswdPolicy.Type.HEXADECIMAL) &&
                           ((length % 2) != 0) ) {
                    return getString(uci.net2.tjado.passwdsafe.R.string.length_even_hex);
                }

            } catch (NumberFormatException e) {
                return getString(uci.net2.tjado.passwdsafe.R.string.invalid_length);
            }
            ...
            if (itsType != PasswdPolicy.Type.HEXADECIMAL) {
                ...
                if (!oneSelected) {
                    return getString(uci.net2.tjado.passwdsafe.R.string.option_not_selected);
                }
            }
                ...
                for (int i = 0; i < itsOptions.length; ++i) {
                    if (itsOptions[i].isChecked()) {
                        try {
                            ...
                            return getString(
                                    uci.net2.tjado.passwdsafe.R.string.invalid_option_length);
                        }
                    }
                }
                if (minOptionsLen > length) {
                    return getString(uci.net2.tjado.passwdsafe.R.string.password_len_short_opt);
                }
            }
            ...
            if (itsUseCustomSymbols.isChecked()) {
                ...
                if (TextUtils.isEmpty(syms)) {
                    return getString(uci.net2.tjado.passwdsafe.R.string.empty_custom_symbols);
                }
                for (int i = 0; i < syms.length(); ++i) {
                    ...
                    if (Character.isLetterOrDigit(c) ||
                        ...
                        return getString(
                                uci.net2.tjado.passwdsafe.R.string.custom_symbol_not_alphanum);
                    }
                }
            }
