 ...
 */
package uci.tjado.passwdsafe.file;
...
public final class PasswdRecordFilter implements Parcelable
{
    ...
    public enum ExpiryFilter
    {
        ...
        public String getRecordsExpireStr(int numRecords, Resources res)
        {
            ...
            String[] strs = res.getStringArray((numRecords == 1) ?
                                               uci.tjado.passwdsafe.R.array.expire_filter_record :
                                               uci.tjado.passwdsafe.R.array.expire_filter_records);
        ...
        case QUERY: {
            if (itsSearchQuery != null) {
                if (QUERY_MATCH_TITLE == null) {
                    QUERY_MATCH_TITLE = ctx.getString(uci.tjado.passwdsafe.R.string.title);
                    QUERY_MATCH_USERNAME = ctx.getString(uci.tjado.passwdsafe.R.string.username);
                    QUERY_MATCH_URL = ctx.getString(uci.tjado.passwdsafe.R.string.url);
                    QUERY_MATCH_EMAIL = ctx.getString(uci.tjado.passwdsafe.R.string.email);
                    QUERY_MATCH_NOTES = ctx.getString(uci.tjado.passwdsafe.R.string.notes);
                }
    ...
    public final String toString(Context ctx)
    {
        ...
        case EXPIRATION: {
            ...
            case EXPIRED: {
                return ctx.getString(uci.tjado.passwdsafe.R.string.password_expired);
            }
            case TODAY: {
                return ctx.getString(uci.tjado.passwdsafe.R.string.password_expires_today);
            }
            case IN_A_WEEK:
            case IN_TWO_WEEKS:
            case IN_A_MONTH:
            ...
            case CUSTOM: {
                return ctx.getString(
                        uci.tjado.passwdsafe.R.string.password_expires_before,
                        Utils.formatDate(itsExpiryAtMillis, ctx,
                                     true, true, false));
            }
            case ANY: {
                return ctx.getString(uci.tjado.passwdsafe.R.string.password_with_expiration);
            }
            }
        }
        }
